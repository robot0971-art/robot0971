...
namespace SunnysideIsland.Animal
{
    ...
    public class ChickenAI : AnimalAI
    {
        ...
        public bool CanLayEggs => _chickenColor == ChickenColor.White;
        ...
        protected override void UpdateWander()
        {
            ...
            if (_wanderTarget != Vector3.zero && _groundLayer != 0 && !IsOnGrass(_wanderTarget))
            {
            ...
            if (_groundLayer != 0 && !IsOnGrass(transform.position))
            {
                SetRandomWanderTarget();
                if (_wanderTarget == Vector3.zero)
                {
            ...
            if (_targetEggPoint != null)
            {
                ...
                if (_groundLayer != 0 && !IsOnGrass(_targetEggPoint.transform.position))
                {
            ...
            else
            {
                ...
                if (_groundLayer != 0 && !IsOnGrass(nextPosition))
                {
        ...
        {
            if (_groundLayer == 0) return true; // Grass 레이어가 없으면 항상 true
            ...
            return Physics2D.OverlapPoint(position, _groundLayer) != null;
        }
        ...
        private void SetRandomWanderTarget()
        {
            ...
            if (_grassTilemap != null)
            {
                ...
                for (int i = 0; i < 50; i++)
                {
                    ...
                    Vector3 candidatePos = transform.position + new Vector3(randomX, randomY, 0);
            ...
            if (_groundLayer != 0)
            {
                for (int i = 0; i < 30; i++)
                {
                    ...
                    if (Physics2D.OverlapPoint(candidatePos, _groundLayer) != null)
                    {
                        _wanderTarget = candidatePos;
                        return;
                    }
            ...
            float fallbackX = (Random.value * 2f - 1f) * _wanderRadius;
            float fallbackY = (Random.value * 2f - 1f) * _wanderRadius;
            _wanderTarget = transform.position + new Vector3(fallbackX, fallbackY, 0);
        }
